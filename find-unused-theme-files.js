...
function readLiquidReferences(filePath) {
	...
	const sectionMatches = [
		...content.matchAll(/['"]section[\/'-]([a-zA-Z0-9_-]+)\.liquid['"]/g),
	].map((m) => m[1] + '.liquid');
	const snippetMatches = [
		...content.matchAll(/{%\s*render\s+['"]([a-zA-Z0-9_-]+)['"]\s*.*?%}/g),
	].map((m) => m[1] + '.liquid');
...
function readJsonReferences(filePath) {
	try {
		...
		console.warn(
			`⚠️  Failed to parse JSON from ${filePath}: ${err.message}`,
		);
	}
}
...
function getUnusedFiles(directory, referencedSet) {
	return fs.existsSync(directory)
		? fs
				...
				.filter(
					(file) =>
						file.endsWith('.liquid') && !referencedSet.has(file),
				)
		: [];
}
